...
export const StepCardGrid: React.FC<StepCardGridProps> = ({
    ...
    const companiesSteps = steps.companies || defaultSteps.companies;
    ...
    if (role === "companies") {
        return (
            <section className={cn("py-16 lg:py-24", className)} id="companies-flow">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <motion.div
                        initial={{ opacity: 0, y: shouldReduceMotion ? 0 : 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                    ...
                    >
                        <h2 className="text-3xl lg:text-4xl font-bold text-foreground mb-4">
                            How Companies Get Insights
                        </h2>
                        <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
                            Access vetted users in 24 hours
                        </p>
                    </motion.div>
                    {renderStepCards(companiesSteps, "companies")}
                </div>
            </section>
        );
    }
...
const MobileTabsSection: React.FC<{
    ...
    companiesSteps: StepItem[];
    renderStepCards: (steps: StepItem[], role: string) => React.ReactNode;
}> = ({ expertsSteps, companiesSteps, renderStepCards }) => {
    const [activeTab, setActiveTab] = React.useState<"experts" | "companies">("experts");

    const TAB_OPTIONS = [
        { value: "experts" as const, label: "For Experts" },
        { value: "companies" as const, label: "For SaaS Companies" },
    ];
        ...
        <>
            ...
            <div className="space-y-8">
                {activeTab === "experts" ? (
                    ...
                    <div>{renderStepCards(companiesSteps, "companies")}</div>
                )}
            </div>
        </>
    );
